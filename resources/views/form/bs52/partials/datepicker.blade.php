...
@endif
<div class="input-group" data-jst-component="flatpickr" id="{{ $id }}" data-jst-field="{{ $name }}">
    <input type="text" id="{{ $id }}Input" class="form-control {{ $class ?? '' }} {{ count($errors->get($name)) ? 'is-invalid' : '' }}"
           ...
           @isset($noValidate) data-jst-no-validate="true" @else @isset($validate) data-jst-validate="{{ $validate }}" @endisset @endisset data-input>
